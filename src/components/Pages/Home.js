...
export default function Home() {
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: "url(https://turkishairlines.ssl.cdn.sdlmedia.com/636866809060120078CY.jpg)",
      }}>
      <div className={isLoading ? "block absolute z-10 bg-turkishtransp w-full h-screen" : "hidden"}>
        <Loader />{" "}
      </div>
      <div className="w-full px-4 pt-4 lg:max-w-screen-lg">
        <div className="hidden py-4 text-center text-4xl text-white md:block">
          {getGreetingMessage()}, <br></br> Where do you want to explore next?
        </div>
        <div className="bg-white md:bg-turkishtransp md:text-white md:pb-4">
          <div className="p-4">
            <label
              className={`${
                roundtrip ? `border-b-3 border-${styleSetting.secondary}` : "border-none"
